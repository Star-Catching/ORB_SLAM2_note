...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        //源码为-usleep(5000); //usleep为Linux下休眠函数,Windows下为Sleep,sleep_for为跨平台休眠函数
...
bool LoopClosing::DetectLoop()
{
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        {
            //hwh 如果函数进到这里，表示系统刚刚发现闭环候选帧或者连续候选帧组断裂，该函数把连续候选帧数置零
            ...
            //hwh 感觉上面两位大佬都不对，因为vCurrentConsistentGroups为局部变量，如果函数能进到这里说明vCurrentConsistentGroups为空
