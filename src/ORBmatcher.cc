        ...
        if(f1it->first == f2it->first)
        {
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                if(bestIdx2>=0)
                {
					...
					//源码没有vbMatched2[bestIdx2]=true; ,确实得加不然可能会重复匹配
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                for(vector<size_t>::const_iterator vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
                {
                    ...
                    //hwh 同感
                    //hwh 不对，虽然在刚进来之前清空了mvpMapPoints，但是在匹配成功之后，会将上一帧的3D点复制给当前帧
                    //hwh      也许会存在赋值过的一个点，这里加了个判断，以第一次为准(这里是否加一个判断，多次匹配时
                    //hwh      判断哪一次更好? 可能是因为在优化时会标记outlier，优化完会剔除，一旦提的3D点少时，应该需要加判断)。
                    //hwh !!!其实这里的目的就是为了赋值当前帧的3D点，因为仅位姿优化时(PoseOptimization)，
                    //hwh    利用的就是3D点与特征点的重投影误差
