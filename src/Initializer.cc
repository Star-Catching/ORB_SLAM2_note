                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // 新建一个容器vAllIndices存储特征点索引，并预分配空间   为了8点发取特征点用的
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // wxz Hn 是用归一化之后的坐标点进行计算得到的单应矩阵，而我们需要得到归一化之前坐标点得到的单应矩阵
        H21i = T2inv*Hn*T1;
		//然后计算逆
        H12i = H21i.inv();
    ...
    const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
{
	...
	//转换成基础矩阵的形式
    //hwh reshape函数对矩阵元素进行序列化,将9维列向量,变成通道为0,行数为3的矩阵即3*3矩阵
    ...
    */
    ...
    //hwh 下面一波操作相当于 把上面算出来的F矩阵进行SVD分解，然后强行满足秩为2的约束，再把分解的合成F',总之就是让基础矩阵满足秩为2的约束
    //hwh 其实上面已经算出F矩阵了，这里只是对F矩阵进行了调整(先分解再组合)，使得其满足秩为2
    ...
    float sigma)                        //估计误差
{
    ...
    for(int i = 0; i < N; i++)
    {
		...
		// 计算投影归一化坐标
        //hwh x2in1 ≌ H12*x2不是相等，只是尺度意义下相等，会相差一个尺度s,w2in1inv是为了去掉这个因子的影响，具体见《十四讲》p171
...
{   
    ...
    // 对于第一个相机是 P1=K*[I|0]  K是内参   
    // wxz 我的理解就是第一个相机的相机坐标系定为世界坐标系下的原点 那么 第一个相机的相机坐标系转到世界坐标系下 就是乘一个单位矩阵
    ...
    P2 = K*P2;
    // 第二个相机的光心在世界坐标系下的坐标
    cv::Mat O2 = -R.t()*t;  // wxz 坐标变换 就用t 没有R
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        if(!isfinite(p3dC1.at<float>(0)) || !isfinite(p3dC1.at<float>(1)) || !isfinite(p3dC1.at<float>(2)))   // isfinite：是有限值
        {
