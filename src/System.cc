							...
							const double &timestamp)	//时间戳
{
    ...
    // Check mode change
    {
        ...
        //hwh 独占锁，主要是为了mbActivateLocalizationMode和mbDeactivateLocalizationMode不会发生混乱
        unique_lock<mutex> lock(mMutexMode);
        //如果激活定位模式
        //hwh mbActivateLocalizationMode为true会关闭局部地图线程
        if(mbActivateLocalizationMode)
        {
            ...
            //告知追踪器，现在 只有追踪工作
            //hwh 局部地图关闭以后，只进行追踪的线程，只计算相机的位姿，没有对局部地图进行更新
            //hwh 设置mbOnlyTracking为真
            mpTracker->InformOnlyTracking(true);// 定位时，只跟踪
            //同时清除定位标记
            //hwh 关闭线程可以使得别的线程得到更多的资源
            mbActivateLocalizationMode = false;// 防止重复执行
        }//如果激活定位模式
        //hwh 如果mbDeactivateLocalizationMode是true，局部地图线程就被释放, 关键帧从局部地图中删除.
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        //hwh  mbActivateLocalizationMode为true 表示 开启仅定位模式，默认为false 通过pangolin窗口触发
        if(mbActivateLocalizationMode)
        {
            mpLocalMapper->RequestStop();
        ...
        //hwh  mbDeactivateLocalizationMode为 true 表示 关闭仅定位模式，默认为false 通过pangolin窗口触发
        if(mbDeactivateLocalizationMode)
        {
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();
    ...
    {
        unique_lock<mutex> lock(mMutexReset);
        //hwh  mbReset true 表示 系统复位，默认为false 通过pangolin窗口触发
        if(mbReset)
        {
            mpTracker->Reset();
            mbReset = false;
