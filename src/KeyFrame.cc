...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    cv::Mat Rwc = Rcw.t();
    // 和普通帧中进行的操作相同
    Ow = -Rwc*tcw;      //hwh Ow为相机中心，可以理解为u-v坐标系(0,0)点在世界坐标系下的坐标   0 = R*p + t --->   p = -R.t()*t
    ...
    Cw = Twc*center;     //hwh 双目相机中心在世界坐标系下的坐标
}
 ...
 */
//TODO hwh-采用一种会自动排序的数据结构会不会更好些，会省掉一些代码量，不过运行效率可能不高，测试一下运行效率
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    //hwh rbegin()返回的迭代器指向的正是最后一个元素的位置，但这个位置和end()的位置是不同的，因为end()指向的是最后一个元素再后面的一个位置（超尾迭代器）
...
void KeyFrame::SetErase()
{
    ...
    //hwh 这里其实是别的函数调用SetBadFlag函数来控制mbToBeErased变量，感觉作者这块儿代码写的不太好
