    ...
    const double &timestamp)    //时间戳
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)// 没有成功初始化的前一个状态就是NO_IMAGES_YET
        //hwh 经过Frame构造函数 主要完成 1.图像特征点提取mvKeys 2.图像去畸变mvKeysUn 3.特征点分配到图像网格中为以后特征匹配做加速mGrid
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD)
            ...
            //单目初始化
            //hwh 单目初始化主要为了 1.设置初始化时第一帧位姿为原点，第二帧为H或F变换求解出的位姿
            //hwh                   2.创建初始化地图点MapPoints
    ...
    else
    {
        ...
        //hwh 感觉这个应该放在TrackLocalMap后面更直观些，因为TrackLocalMap里面会将最强的共视关键帧作为当前帧的参考帧
        ...
        if(bOK)
        {
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    ...
                    //hwh 三个跟踪方法(恒速跟踪、关键帧跟踪、重定位跟踪)，都是将已有的MapPoint进行跟踪，已有的MapPoint肯定Observations大于等于1
                    //hwh 初始化时会生成MapPoint，在LocalMapping中也会生成MapPoint
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            //hwh 秒啊,算一个技巧吧,再delete之前先判断下是否删除,防止发生段错误
            if(mpInitializer)
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    //hwh 因为初始化时地图点是利用pKFini和pKFcur求得的，所以pKFini和pKFcur关键帧的地图点都一样
...
bool Tracking::TrackLocalMap()
{
    ...
    //? 这里的跟踪怎么理解?
    //hwh 理解应该是：当前关键帧中的实际看到的地图点会小于理论上应该看到的地图点，(可能是视角或光照影响没看见)
    //hwh            所以作者将局部地图中所有的地图点投影到当前帧中，并选取当前帧视角内的地图点进行重新投影匹配来获得更多的地图点
    SearchLocalPoints();
...
void Tracking::UpdateLocalMap()
{
    ...
    // Update
    // 更新局部关键帧和局部MapPoints
    //hwh 维护mvpLocalKeyFrames局部地图关键帧变量，该变量保存了  1.当前帧的共视关键帧p1   2.p1的最强共视关键帧p2  3.p1的父关键帧p3    4.p1的子关键帧p4
    //hwh                                                    因为父关键帧就是共视程度最强的共视关键帧，所以p3就是p2
    UpdateLocalKeyFrames();
    //hwh 维护mvpLocalMapPoints局部地图点变量，该变量保存了mvpLocalKeyFrames里所有的地图点
    UpdateLocalPoints();
}
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    //? 提问:为什么要乘3呢? 
    //hwh 回答：1.当前帧的共视关键帧p1   2.p1的最强共视关键帧p2  3.p1的父关键帧p3    4.p1的子关键帧p4
    //hwh       因为父关键帧就是共视程度最强的共视关键帧，所以p3就是p2
            ...
            if(!Tcw.empty())
            {
                ...
                if(nGood<50)
                {
                    ...
                    if(nadditional+nGood>=50)
                    {
                        ...
                        {
                            //重新进行搜索  // Step 5：通过PoseOptimization对姿态进行优化求解
                //只优化位姿,不优化地图点的坐标;返回的是内点的数量
                int nGood = Optimizer::PoseOptimization(&mCurrentFrame);
