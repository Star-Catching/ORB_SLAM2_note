     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    if(mbInitialComputations)
    {
		...
		//源码里为mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/(mnMaxX-mnMinX); //源码对,变量本来就是float,不需要转换,而且第一个变量已经转换为float,第二个变量如果是int型,编译器会自动转换
        mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);
		// 表示一个图像像素相当于多少个图像网格行（高）
		//源码里为mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/(mnMaxY-mnMinY);//源码对,变量本来就是float,不需要转换,而且第一个变量已经转换为float,第二个变量如果是int型,编译器会自动转换
     ...
     mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    if(mvKeys.empty()){
        cout << "mvKeys is empty" << endl;
        return;
    }

    //*****动态点剔除
    for(int i = 0; i < N; i++){
        if(my_socket.detect_point(mvKeys[i].pt.x,mvKeys[i].pt.y,1)){
            //动态点坐标变为(-1,-1)
...
void Frame::AssignFeaturesToGrid()
{
	...
	// ? 这里0.5 是为什么？节省空间？
    // FRAME_GRID_COLS = 64，FRAME_GRID_ROWS=48  默认640×480的分辨率
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    if(flag==0)
        ...
        // 这里使用了仿函数来完成，重载了括号运算符 ORBextractor::operator()   需要手动跳转
...
void Frame::UndistortKeyPoints()
{
    ...
    // Step 3 存储校正后的特征点
    mvKeysUn.resize(N); // gpt resize(N) 的目的是将容器 mvKeysUn 的大小调整为 N。
	//遍历每一个特征点
    for(int i=0; i<N; i++)
    {
		//根据索引获取这个特征点
...
void Frame::ComputeStereoMatches() {
        ...
        // 为左图每一个特征点il，在右图搜索最相似的特征点ir   粗匹配
        for (int iL = 0; iL < N; iL++) {
            ...
            if (bestDist < thOrbDist) {
                ...
                //hwh 理解成数学问题就是求一个函数的极小值，如果最小值出现在边界，那么可以看做该函数是单调函数，边界最小不能看成极小值
                //hwh 最小值出现在区域中间时，可以看成函数不是单调函数，有增区间有减区间，最小值就是极小值
                // 搜索窗口越界判断ß
